...
- `React.useState` accepts an initial value (`React.useState(0)`). It  returns an array that we destructure to get the state and a setter function to update that state:

```javascript
const [count, setCount] = React.useState(0)
```

You can then update the state (`count`) using that setter function (`setCount`) like this:

```javascript
const increment = () => setCount(count + 1)
```

We can then just call that arrow function whenever we need to trigger the state. In this specific example it would work like this:
