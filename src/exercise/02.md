...
- React is the package for handling element creation
- ReactDOM is a package for displaying those elements in a DOM (webpage)
- There are other packages for displaying elements in apps (React Native) and VR (React VR) so these are separate to React itself.
- React APIs are laborious!
- Creating each individual element is easier since one method handles all props of the element
- Children of an element would be an array inside the parent method.
